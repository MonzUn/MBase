...
namespace MEngineInput
{
	...
	// Text input
	std::string* TextInputStringReference = nullptr;
...
void MEngineInput::StartTextInput(std::string* textInputString)
{
	...
	TextInputStringReference = textInputString;
	TextInputCaretIndex = textInputString->length();
}

void MEngineInput::StopTextInput()
{
	if (TextInputStringReference != nullptr)
	{
		SDL_StopTextInput();
		TextInputStringReference = nullptr;
...
{
	ScanCodeToMKeyConversionTable = new std::unordered_map<uint32_t, MENGINE_KEY>();
	SDLScanCodeToMKeyConversionTable = new std::unordered_map<SDL_Scancode, MENGINE_KEY>();
...
{
	delete ScanCodeToMKeyConversionTable;
	delete SDLScanCodeToMKeyConversionTable;
}
	...
	{	
		if (sdlEvent.key.keysym.sym == SDLK_BACKSPACE) // Remove last character
		{
			if (sdlEvent.key.state == SDL_PRESSED && TextInputCaretIndex > 0)
				TextInputStringReference->erase(--TextInputCaretIndex, 1);
		...
		else if (sdlEvent.key.keysym.sym == SDLK_c && SDL_GetModState() & KMOD_CTRL) // Clipboard copy
		{
			if (sdlEvent.key.state == SDL_PRESSED)
				SDL_SetClipboardText(TextInputStringReference->c_str());
		...
		else if (sdlEvent.key.keysym.sym == SDLK_v && SDL_GetModState() & KMOD_CTRL) // Clipboard paste
		{
			if (sdlEvent.key.state == SDL_PRESSED)
			{
				...
				TextInputStringReference->insert(TextInputCaretIndex, clipboardString);
		...
		{
			if (sdlEvent.key.state == SDL_PRESSED)
				TextInputCaretIndex = TextInputStringReference->length();
		...
		{
			if (sdlEvent.key.state == SDL_PRESSED && TextInputCaretIndex < TextInputStringReference->length())
				++TextInputCaretIndex;
		}
		else if (sdlEvent.type == SDL_TEXTINPUT)
		{
			TextInputStringReference->insert(TextInputCaretIndex++, sdlEvent.text.text);
			consumedEvent = true;
		}
	}
