...
#define MUTILITY_LOG_CATEGORY_COMPONENT_MANAGER "ComponentManager"
...
MEngineComponentMask MEngineComponentManager::RegisterComponentType(const MEngine::Component& templateComponent, uint32_t templateComponentSize, uint32_t maxCount, const char* componentName)
{
	...
	{
		MLOG_WARNING("Ran out of component mask IDs when attempting to add component \"" << componentName << '"', MUTILITY_LOG_CATEGORY_COMPONENT_MANAGER);
...
bool MEngineComponentManager::UnregisterComponentType(MEngineComponentMask componentType) // TODODB: Destroy active components
{
	...
	{
		MLOG_WARNING("Attempted to unregister an invalid component mask; componentMask = " << MUtility::BitSetToString(componentType), MUTILITY_LOG_CATEGORY_COMPONENT_MANAGER);
		return false;
	}
	else if (!m_IDBank.IsIDActive(componentType))
	{
		MLOG_WARNING("Attempted to unregister an inactive component mask; componentMask = " << MUtility::BitSetToString(componentType), MUTILITY_LOG_CATEGORY_COMPONENT_MANAGER);
		return false;
	}
#endif
	...
	for (int i = 0; i < m_Buffers->size(); ++i)
	{
		if ((*m_Buffers)[i].ComponentMask == componentType)
		{
			...
			{
				MLOG_WARNING("Failed to return the component mask for component \"" << (*m_Buffers)[i].ComponentName << "\"; the component type will not be unregistered", MUTILITY_LOG_CATEGORY_COMPONENT_MANAGER);
				return false;
			}
		}
	}

	MLOG_ERROR("Failed to find the component buffer for component mask " << MUtility::BitSetToString(componentType), MUTILITY_LOG_CATEGORY_COMPONENT_MANAGER);
	return false;
}
...
MUtility::Byte* MEngineComponentManager::GetComponentBuffer(MEngineComponentMask componentType, int32_t& outComponentCount)
{
	...
	if (componentType == INVALID_MENGINE_COMPONENT_MASK)
	{
		MLOG_WARNING("Attempted to get buffer using an invalid component mask; componentMask = " << MUtility::BitSetToString(componentType), MUTILITY_LOG_CATEGORY_COMPONENT_MANAGER);
		outComponentCount = -1;
		return nullptr;
	}
	else if (!m_IDBank.IsIDActive(componentType))
	{
		MLOG_WARNING("Attempted to get buffer using an inactive component mask; componentMask = " << MUtility::BitSetToString(componentType), MUTILITY_LOG_CATEGORY_COMPONENT_MANAGER);
		outComponentCount = -1;
		return nullptr;
	}
#endif
